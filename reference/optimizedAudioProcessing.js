/**
 * Optimized Audio Processing Module
 * 
 * This module provides optimized functions for audio processing in the chatbot,
...
async function _backgroundVectorFetch(vectorStore, question, indexType) {
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Vector retrieval timeout')), 2000); // Reduced timeout
    });
